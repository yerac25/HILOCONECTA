(function () {
  ...
  function renderProductCard(p) {
    return (
      ...
      '<div class="product-card__img-wrap"><img class="product-card__img" src="' + p.image + '" alt="' + p.name + '" loading="lazy" referrerpolicy="no-referrer" onerror="this.onerror=null;this.src=\'' + (p.fallback || HiloData.getProductFallback(p.id) || HiloData.getCategoryImage(p.category)) + '\'"></div>' +
  ...
  function renderCategories() {
    ...
    var cards = CATEGORIES.map(function (c) {
      return (
        ...
        '<img class="category-card__img" src="' + c.image + '" alt="' + c.name + '" loading="lazy">' +
        '<h3>' + c.name + '</h3>' +
